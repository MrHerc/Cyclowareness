"""The sandbox engine: the properties that make its verdicts trustworthy.
...
from app.sandbox.engine import identify, pipeline, report, scoring
from app.sandbox.engine.contracts import AnalyzerResult, IOCs, Sample, Signal, risk_level
from app.sandbox.engine.fetcher import UnsafeURL, assert_safe
...
def _analyse(name: str, data: bytes):
    from app.sandbox.engine import analyzers
...
def test_no_analyzer_ever_shells_out_with_sample_data():
    ...
    Comments are stripped before the scan, and only comments. An analyzer that
    detects a threat has to be able to NAME it — `scripts.py` carries a note
    about the sixteen `L.exec(t)` matches that once made the engine call jQuery
    malware — and a test that cannot tell an accusation from a confession makes
    that note unwriteable. Stripping string literals too would be the wrong
    trade: a shell command assembled in a string is exactly what this is looking
    for, so those stay in scope.
    """
    import io
    import pathlib
    import re
    ...
    for path in root.rglob("*.py"):
        ...
        # Blank out comment spans in place, so reported line numbers stay true
        # to the file an engineer will open.
        chars = list(text)
        ...
        try:
            for token in tokenize.generate_tokens(io.StringIO(text).readline):
                ...
                begin = starts[row - 1] + col
                stop = starts[end_row - 1] + end_col
                chars[begin:stop] = " " * (stop - begin)
        except (tokenize.TokenError, IndentationError, SyntaxError) as exc:
            # A file this cannot tokenise is a file this cannot clear. Fail
            # loudly rather than skipping it and reporting a clean scan.
            offenders.append(f"{path.name}: could not be tokenised ({type(exc).__name__})")
            continue
        code = "".join(chars)
...
def test_hostile_input_never_raises(name, data):
    if not data:
        ...
        from app.sandbox.engine.storage import EmptySample
...
def test_zip_slip_and_double_extension_are_flagged():
    from app.sandbox.engine import archives
